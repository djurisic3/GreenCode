...
export function markForLoopsPy(document: vscode.TextDocument) {
  ...
  if (text.includes("for")) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      decorations.push({
        range: new vscode.Range(start, end),
        hoverMessage:
          "The function append in a for loop is much more energy-consuming than a list comprehension",
      });
    }
    ...
    while ((matchString = regexString.exec(text)) !== null) {
      ...
      decorations.push({
        range: new vscode.Range(start, end),
        hoverMessage:
          "String concatenation is eco-friendlier with the join function.",
      });
    }
    ...
    while ((matchForIf = regexForIf.exec(text)) !== null) {
      ...
      decorations.push({
        range: new vscode.Range(start, end),
        hoverMessage:
          "Using the in operator to check if an item is/is not in a list is slower than using a set.",
      });
    }

    let matchForDictLoop;
    const regexForDict =
      /for\s+(\w+)\s+in\s+(\w+)\.keys\(\)\s*:[\s\S]*(\w+)\s*=\s*\2\[\1\]/gim;
    ...
    while ((matchForDictLoop = regexForDict.exec(text)) !== null) {
      ...
      decorations.push({
        range: new vscode.Range(start, end),
        hoverMessage:
          "Looping a dictionary is eco-friendier with items() than keys()",
      });
    }
  }
  return decorations;
...
export function markMiscellaneousPy(document: vscode.TextDocument) {
  if (document.languageId !== "python") {
    return [];
  }
...
export function markCsvPy(document: vscode.TextDocument) {
  if (document.languageId !== "python") {
    return [];
  }
